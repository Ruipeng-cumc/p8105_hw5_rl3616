...
```
    ...
    ## ℹ In argument: `prop_test = map2(n_unsolved, total, ~broom::tidy(prop.test(.x,
    ##   .y)))`.
...
![](p8105_hw5_rl3616_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
Note: Tulsa, AL had only one homicide in the dataset. Because of the
extremely small sample size, the corresponding confidence interval spans
nearly the entire range and provides little interpretive value. For this
reason, I removed Tulsa, AL from the visualization to improve
readability.
